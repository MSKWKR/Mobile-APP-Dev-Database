...
def _search_by_term(term: str, country: str) -> list[str]:
    """
    Search Google Play by term, return deduplicated app IDs.
    Searches in both English and the country's local language to maximise coverage.
    Max 30 results per search (Google's hard cap).
    """
    seen: set[str] = set()
    app_ids: list[str] = []
    ...
    for lang in langs:
        try:
            results = search(term, lang=lang, country=country, n_hits=N_HITS)
            for r in results:
                aid = r.get("appId")
            ...
            print(f"[WARN] search '{term}' lang={lang} ({country}): {e}")
...
def _process_app(app_id: str, category_desc: str, country: str) -> str | None:
    try:
        app_info = _fetch_app_details(app_id, country)
        if not app_info:
            return None
...
def _bulk_insert(app_ids: list[str], category_desc: str, country: str) -> int:
    """Fetch details and insert a list of app IDs using a thread pool."""
    inserted = 0
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        futures = {
            executor.submit(_process_app, aid, category_desc, country): aid
            for aid in app_ids
        }
...
def _run_category_sweep(country: str, seen_app_ids: set[str]) -> None:
    ...
    for i, (category_id, category_desc) in enumerate(CATEGORY_PAIRS):
        ...
        if not new_ids:
            ...
            count = _bulk_insert(new_ids, category_desc, country)
...
def _run_keyword_sweep(country: str, seen_app_ids: set[str]) -> None:
    ...
    for i, term in enumerate(search_terms):
        ...
        if new_ids:
            count = _bulk_insert(new_ids, term, country)
            seen_app_ids.update(new_ids)
...
def _run_language_sweep(country: str, seen_app_ids: set[str]) -> None:
    """
    Search curated verticals in every language from languages.json.
    Bigrams are language-agnostic so we skip them here — only verticals benefit
    from being searched in French, Japanese, Arabic, etc.
    """
    ...
    for li, lang in enumerate(ALL_LANGS):
        for term in VERTICALS:
            try:
                results = search(term, lang=lang, country=country, n_hits=N_HITS)
                app_ids = [r["appId"] for r in results if r.get("appId")]
                new_ids = [aid for aid in app_ids if aid not in seen_app_ids]
                ...
                print(f"[WARN] lang={lang} '{term}' ({country}): {e}")
